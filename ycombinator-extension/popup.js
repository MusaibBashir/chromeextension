...
$('scrapeBtn').addEventListener('click', async () => {
    ...
    chrome.tabs.sendMessage(tab.id, { action: 'SCRAPE_PAGE' }, response => {
        ...
        if (chrome.runtime.lastError) { showMessage('Navigate to a YC Jobs page first', 'error'); return; }
        if (response?.success) {
            scrapedJobs = response.jobs; $('jobCount').textContent = response.count;
            $('sendBtn').disabled = response.count === 0; updateJobList(scrapedJobs);
            showMessage(response.count > 0 ? `Found ${response.count} jobs!` : 'No jobs found on this page.', response.count > 0 ? 'success' : 'error');
        }
    });
});
