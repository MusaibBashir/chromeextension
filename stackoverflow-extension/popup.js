/**
 * Stack Overflow Jobs Scraper - Popup Script
 */
...
$('scrapeBtn').addEventListener('click', async () => {
    ...
    chrome.tabs.sendMessage(tab.id, { action: 'SCRAPE_PAGE' }, response => {
        ...
        if (chrome.runtime.lastError) {
            showMessage('Navigate to Stack Overflow Jobs first', 'error');
            return;
        }
        ...
        if (response?.success) {
            ...
            showMessage(response.count > 0 ? `Found ${response.count} jobs!` : 'No jobs found. Try searching first.', response.count > 0 ? 'success' : 'error');
        }
    });
});
