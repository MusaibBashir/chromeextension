/**
 * Monster Jobs Scraper - Popup Script
 */
...
function updateJobList(jobs) {
    $('jobList').innerHTML = jobs.slice(0, 10).map(job => `
        <div class="job-item">
            ...
            ${job.salary ? `<div class="job-extra">${escapeHtml(job.salary)}</div>` : ''}
        </div>
...
async function checkApiHealth() {
    ...
    $('apiStatus').className = 'value pending';

    chrome.runtime.sendMessage({ action: 'CHECK_API' }, response => {
        if (response?.success) {
...
$('scrapeBtn').addEventListener('click', async () => {
    ...
    chrome.tabs.sendMessage(tab.id, { action: 'SCRAPE_PAGE' }, response => {
        ...
        if (chrome.runtime.lastError) {
            showMessage('Navigate to Monster.com first', 'error');
            return;
        }

        if (response?.success) {
            scrapedJobs = response.jobs;
            $('jobCount').textContent = response.count;
            $('sendBtn').disabled = response.count === 0;
            updateJobList(scrapedJobs);
            showMessage(response.count > 0 ? `Found ${response.count} jobs!` : 'No jobs found. Try scrolling down.', response.count > 0 ? 'success' : 'error');
        }
    });
});
...
$('sendBtn').addEventListener('click', () => {
    ...
    chrome.runtime.sendMessage({ action: 'SEND_TO_API', jobs: scrapedJobs }, response => {
        ...
        if (response?.success) {
            ...
            showMessage(`Sent! Created: ${r.created}, Updated: ${r.updated}`);
            scrapedJobs = [];
            ...
            showMessage(response?.error || 'Failed to send', 'error');
        }
    });
});
...
chrome.storage.local.get(['lastScrapedJobs'], result => {
    if (result.lastScrapedJobs?.length) {
        scrapedJobs = result.lastScrapedJobs;
